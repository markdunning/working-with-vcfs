...
Lets go through an example of how `BSgenome` can help us with the first coding variant.

```{r}
locs[1]
common.coding.ranges[1]
```


If we view this variant in `IGV` we will see the that position is the third base in a codon. So if we want the sequence of the whole codon we have to go two bases upstream

```{r}
locs.codon <- resize(common.coding.ranges[1],3,fix = "end")
```

The `getSeq` function can be used to obtain the genome sequence at this coordinates.

```{r}
library(BSgenome.Hsapiens.UCSC.hg19)
codon.seq <- getSeq(BSgenome.Hsapiens.UCSC.hg19, locs.codon)
codon.seq
codon.seq[[1]]
```

`Biostrings` has a `translate` function. The result should agree with what you see in IGV.

```{r}
translate(codon.seq[[1]])
```

Now what happens if we replace the variant base with it's alternate?

```{r}
translate(replaceLetterAt(codon.seq[[1]],3,"C"))
```

In this case we don't observe any change in amino acid, so this is an example of a *synonymous* variant. However, repeating for the second coding variants does reveal a change; a *non-synonymous* variant.

(we need to take care here as the variant observed is on the *reverse* strand)

```{r}
locs[2]
common.coding.ranges[2]
locs.codon <- resize(common.coding.ranges[2],3,fix = "start")
locs.codon
codon.seq <- getSeq(BSgenome.Hsapiens.UCSC.hg19, locs.codon)
codon.seq
translate(complement(codon.seq[[1]]))
translate(complement(replaceLetterAt(codon.seq[[1]],3,"A")))
```

Although it is useful to see what might be going-on under the hoof, we clearly we cannot go through all variants in such detail. Fortunately, there is a function, `predictCoding` that will do all the predictions for us.

...
The meaning of each `KEY`  is given in the header for the `.vcf` file. For instance, one item of interest is `DP` which is described in the header as:- 
...
```{}
freebayes -f ../ref_data/human_g1k_v37.fasta --region 20:60000-10060000 data/hapmap/NA12878.chr20.bam data/hapmap/NA12873.chr20.bam data/hapmap/NA12874.chr20.bam > combined.chr20.subset.freebayes.vcf
```
...
As described above, the `INFO` column in a `.vcf` file gives per-variant metadata as a series of key-value pairs. We can interrogate these data using the `info` function, which returns a data-frame-like object.
...
`NS` represents the number of samples with data. We would expect this to be 3 in all cases. However, as we are dealing with low-coverage data there are some sites
...
`DP` gives the depth observed at each position
...
```{r}
infoMat[numVars>1,1:5]
```

Select only positions where a single type of variant was found

```{r}
hapmap.vcf.singleVar <- hapmap.vcf[numVars==1]
```
with this new object, we need to create a new info matrix


```{r}
infoMat.new <- info(hapmap.vcf.singleVar)
```
...
Do we still have any variants without data in all samples?

- Create a new object where data was observed in all samples
    + call it `hapmap.vcf.singleVar.fullData`
******

```{r echo=FALSE}
table(info(hapmap.vcf.singleVar)$NS)
hapmap.vcf.singleVar.fullData <- hapmap.vcf.singleVar[infoMat.new$NS==3]
```


## Looking at genotypes

`geno` gives the genotype information
```{r}
geno(hapmap.vcf.singleVar.fullData)
head(geno(hapmap.vcf.singleVar.fullData)$GT)

```

```{r}
called <- apply(geno(hapmap.vcf.singleVar.fullData)$GT, 2,function(x) x !="0/0")
limma::vennDiagram(called)
```

?? some positions with no variants called ??
...
As we have seen already with other Bioconductor objects, typing the name of the object will print a summary to the screen.
```{r}
freebayes.calls
```

The "header"" information can be extracted using `header`. This will contain the definitions of all the per-genotype and per-sample information stored in the file
...
```{r echo=FALSE}
info(header(freebayes.calls))[1:3,]
geno(header(freebayes.calls))[1:3,]
```
...
```{r}
freebayes.info <- info(freebayes.calls)
freebayes.info
```

...
With a bit of thought, it would be possible to perform overlaps with the exon structure described in the database. However, there is a convenience function `locateVariants` that will do the hard work for us. `CodingVariants` tells the function to look for variants within coding sequences
...
```{r}
coding.ranges <- rowRanges(common.coding.snv)
```
...
perExonSnp <- countOverlaps(exons.20, coding.ranges)
```
...
coding <- predictCoding(common.coding.snv, txdb, seqSource=Hsapiens)
